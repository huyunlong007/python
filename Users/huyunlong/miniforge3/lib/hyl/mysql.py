...
try:
    ...
    with conn.cursor(cursor=pymysql.cursors.DictCursor) as cursor:  # 将默认的输出元组改为输出为字典
        ...
        sql = "CREATE TABLE class(title  CHAR(20),sec_title  CHAR(20),content  VARCHAR(6499));"
        cursor.execute(sql)

        into = "INSERT INTO class(title,sec_title,content) VALUES (%s,%s, %s);"
        values =(4, 'red', 28)
        change_values = (5, 'black', 27)
        cursor.execute(into, change_values)
        # 第四步：提交上面的操作
        conn.commit()
